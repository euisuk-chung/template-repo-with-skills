...
Usage:
    cd <canonical-skill-directory>/scripts
...
def render(
    ...
    try:
        ...
        print("Run setup from the canonical skill's scripts directory: uv sync && uv run playwright install chromium", file=sys.stderr)
        sys.exit(1)
    ...
    with sync_playwright() as p:
        try:
            ...
            if "Executable doesn't exist" in str(e) or "browserType.launch" in str(e):
                ...
                print("From the canonical skill's scripts directory, run: uv run playwright install chromium", file=sys.stderr)
                sys.exit(1)
            raise
