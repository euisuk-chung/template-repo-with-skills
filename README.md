# Shared Agent Skills Template

Codex와 Claude Code가 하나의 저장소 스킬셋을 공유하도록 구성한 템플릿입니다.
실제 스킬은 `.agents/skills`에서만 관리하고, Claude Code가 발견할 수 있도록
`.claude/skills`에는 중앙 원본을 읽는 텍스트 어댑터를 생성합니다.

심볼릭 링크를 사용하지 않으므로 Windows의 Developer Mode나 Git
`core.symlinks` 설정 없이 Linux와 Windows에서 같은 구조로 동작합니다.
...
├── .github/workflows/              # Linux/Windows 일관성 검사
├── _notes/                         # 에이전트가 작성한 공유 작업 문서
├── scripts/sync_skills.py          # 어댑터·카탈로그·lock 생성 및 검증
├── scripts/update_shared_skills.py # 템플릿에서 공용 스킬 갱신
├── AGENTS.md                       # 저장소 공통 에이전트 지침
└── CLAUDE.md                       # AGENTS.md를 가져오는 Claude 진입점
```
...
1. 이 템플릿으로 새 저장소를 만듭니다(GitHub "Use this template" 또는 복사).
...
갱신 스크립트는 `origin: shared` 스킬, 스킬 도구(`scripts/`, `tests/`, CI 워크플로,
스킬 README)를 덮어쓰고, `_notes/**/README.md`는 없을 때만 생성합니다. `local` 스킬과
스킬 안의 `.venv` 같은 로컬 환경은 건드리지 않습니다. 출처 URL·ref·커밋은
`.agents/skills.lock`의 `shared_source`에 기록됩니다.
...
이번 안전성 수정 이전 updater가 이미 설치된 저장소는 먼저 신뢰하는 원본의
`scripts/sync_skills.py`와 `scripts/update_shared_skills.py`를 함께 교체한 후 실행하세요.
이미 실행 중인 구버전 updater 자체에 새 안전장치를 소급 적용할 수는 없습니다.
...
# Windows PowerShell
py -3 scripts/sync_skills.py --check
...
GitHub Actions는 같은 검사와 테스트를 Ubuntu와 Windows에서 실행합니다.
Python 최소 지원 버전 `3.10`과 최신 `3.x`를 각각 검사합니다.
...
해시는 UTF-8 텍스트로 확인된 다음 형식의 CRLF/CR을 LF로 정규화합니다.
`md`, `txt`, `py`, `sh`, `ps1`, `json`, `yaml`, `yml`, `toml`, `lock`, `html`, `css`,
`js`, `ts`, `svg` 및 `.gitignore`, `.gitattributes`, `.editorconfig`입니다.
...
- Claude Code 전용 `$ARGUMENTS`, 명령 주입, 컨텍스트 옵션은 중앙 본문에서 직접
  ...
  사용합니다.
- 문서 언어: 에이전트가 읽는 파일(`AGENTS.md`, 스킬)은 영어, 사람이 읽는 안내
  (`README.md`, `_notes/**/README.md`)는 한국어로 씁니다.
